...
quad_tree::quad_tree(const aabb & a_boundry, int a_capacity /*= 4*/) :
	m_boundry(a_boundry), m_capacity(a_capacity)
{
}
...
bool quad_tree::insert(boid * a_boid)
{
	...
	else
	{
		...
		// Inset into the correct quad tree/s.
...
void quad_tree::sub_divide()
{
	...
	this->m_north_east = new quad_tree(aabb({ x + width / 2, y - height / 2 },
		{ this->m_boundry.get_extents().x + width / 2, this->m_boundry.get_extents().y - height / 2 }), 
		this->m_capacity);
	this->m_north_west = new quad_tree(aabb({ x - width / 2, y - height / 2 },
		{ this->m_boundry.get_extents().x - width / 2, this->m_boundry.get_extents().y - height / 2 }), 
		this->m_capacity);
	this->m_south_east = new quad_tree(aabb({ x + width / 2, y + height / 2 },
		{ this->m_boundry.get_extents().x + width / 2, this->m_boundry.get_extents().y + height / 2 }), 
		this->m_capacity);
	this->m_south_west = new quad_tree(aabb({ x - width / 2, y + height / 2 },
		{ this->m_boundry.get_extents().x - width / 2, this->m_boundry.get_extents().y + height / 2 }), 
		this->m_capacity);
...
bool quad_tree::contains(const aabb & a_range, const boid * a_boid)
{
	return !a_boid->get_position().x  <= a_range.get_extents().x &&
		a_boid->get_position().x >= a_range.get_extents().x &&
		a_boid->get_position().y <= a_range.get_extents().y &&
		a_boid->get_position().y >= a_range.get_extents().y;
}

bool quad_tree::contains(const circle & a_range, const boid * a_boid)
{
	collision_manager::circle_to_circle(a_range, circle(a_boid->get_position(), a_boid->get_radus()));
}
